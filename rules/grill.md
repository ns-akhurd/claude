...
**14.12 External Reference Enrichment** — During Phase 0 of any grill: MUST grep all artifacts for Jira ticket IDs (`[A-Z]+-\d+`) and Confluence URLs (`https://netskope.atlassian.net/wiki/`). For each found: MUST invoke `eng-skills:jira` or `eng-skills:confluence` skill and fetch actual content. Open ticket claimed as resolved → `[GAP]`. Ticket/Confluence content contradicting artifact claims → `[CRITICAL]`. NEVER treat opaque external references as verified context.
